...
namespace calc {
  ...
  protected:
    ...
    map_fun1 m_unary_fun;
    map_fun2 m_binary_fun;
    map_real m_variables;

  public:

    void init() {
      m_unary_fun["cos"]    = my_cos;
      m_unary_fun["sin"]    = my_sin;
      m_unary_fun["tan"]    = my_tan;
  
      m_unary_fun["acos"]   = my_acos;
      m_unary_fun["asin"]   = my_asin;
      m_unary_fun["atan"]   = my_atan;
  
      m_unary_fun["cosh"]   = my_cosh;
      m_unary_fun["sinh"]   = my_sinh;
      m_unary_fun["tanh"]   = my_tanh;
  
      m_unary_fun["exp"]    = my_exp;
      m_unary_fun["log"]    = my_log;
      m_unary_fun["log10"]  = my_log10;
      m_unary_fun["sqrt"]   = my_sqrt;
      m_unary_fun["ceil"]   = my_ceil;
      m_unary_fun["floor"]  = my_floor;
  
      m_binary_fun["atan2"] = my_atan2;
      m_binary_fun["pow"]   = my_pow;
  ...
  class Calculator : public CalculatorFun<T_type> {
    ...
    using CalculatorFun<T_type>::m_unary_fun;
    using CalculatorFun<T_type>::m_binary_fun;
    using CalculatorFun<T_type>::m_variables;
  ...
  Calculator<T_type,U_type>::Calculator(void) : CalculatorFun<value_type>() {
    ...
    m_binary_fun["max"] = max;
    m_binary_fun["min"] = min;
  ...
  Calculator<T_type,U_type>::report_error(ostream& s) {
    ...
    case Unknown_Variable:
      s << "unknown variable: " << tv << '\n';
      break;
      
    case Bad_Position:
      s << "bad position for token: ``" << tv << "''\n";
      break;
    }
    s << "           : " << string_in << "\n           : ";
  ...
  Calculator<T_type,U_type>::get(char const * const s, bool & exist) {
    map_real_const_iterator ii = m_variables.find(s);
    exist = ii != m_variables.end();
  ...
  Calculator<T_type,U_type>::print(ostream & s) const {
    ...
    s << "\nUNARY FUNCTIONS\n";
    for ( f1 = m_unary_fun.begin(); f1 != m_unary_fun.end(); ++f1 )
      s << f1 -> first << ", ";
    
    s << "\n\nBINARY FUNCTIONS\n";
    for ( f2 = m_binary_fun.begin(); f2 != m_binary_fun.end(); ++f2 )
      s << f2 -> first << ", ";
    
    s << "\n\nVARIABLES\n";
    for ( ii = m_variables.begin(); ii != m_variables.end(); ++ii )
  ...
  Calculator<T_type,U_type>::G0() {
    ...
    if ( tt == Variable ) {
      ...
      if ( tt == Assign ) {  // handle assign
        ...
        return m_variables[name] = G1(); // assign value
      }
    }
  ...
  Calculator<T_type,U_type>::G5() {
    ...
    if ( tt == OpenPar ) { // handle ( ... )
      ...
      map_real_iterator iv = m_variables.find(tv);
      map_fun1_iterator f1 = m_unary_fun.find(tv);
      map_fun2_iterator f2 = m_binary_fun.find(tv);
      
      if ( iv != m_variables.end() ) {
        val = iv -> second;
        Next_Token(); // eat token
      } else if ( f1 != m_unary_fun.end() ) {
        Next_Token(); // expect (
